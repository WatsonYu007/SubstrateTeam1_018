...
pub mod pallet {
	...
	#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug)]
	pub struct PolkadotPayload<Public, BlockNumber> {
		block_number: BlockNumber,
		price_usd: (u64, Permill),
		public: Public,
	}

	impl<T: SigningTypes> SignedPayload<T> for PolkadotPayload<T::Public, T::BlockNumber> {
		fn public(&self) -> T::Public {
			self.public.clone()
		}
	}
	...
	pub enum Event<T: Config> {
		...
		NewPolkadot(Option<T::AccountId>, (u64, Permill)),
	}
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_polkadot_unsigned_with_signed_payload(origin: OriginFor<T>, payload: PolkadotPayload<T::Public, T::BlockNumber>,
			_signature: T::Signature) -> DispatchResult
		{
			let _ = ensure_none(origin)?;
			// we don't need to verify the signature here because it has been verified in
			//   `validate_unsigned` function when sending out the unsigned tx.
			let PolkadotPayload { block_number, price_usd,public } = payload;
			log::info!("#?#?#__submit_polkadot_unsigned_with_signed_payload: ({:?}, {:?})", block_number, public);
			Self::append_or_replace_polkadot(price_usd);

			Self::deposit_event(Event::NewPolkadot(None, price_usd));
			Ok(())
		}
	}

				// {"data":{"id":"polkadot","rank":"10","symbol":"DOT","name":"Polkadot","supply":"1076361812.8378700000000000","maxSupply":null,"marketCapUsd":"21275898965.8412006524238959","volumeUsd24Hr":"1554826066.5945891503468904","priceUsd":"19.7664936753436665","changePercent24Hr":"-14.1412846205367241","vwap24Hr":"21.5534663410108214","explorer":"https://polkascan.io/polkadot"},"timestamp":1642818407868}
	impl<T: Config> Pallet<T> {
		...
		fn append_or_replace_polkadot(price: (u64, Permill)) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back(price);
				log::info!("#?#?#__polkadot vector: {:?}", prices);
			});
		}

		fn fetch_price_info(block_number: T::BlockNumber) -> Result<(), Error<T>> {

			log::info!("#?#?#__start fetch_price_info");

			// Self::fetch_polkadot_info(block_number);
			// TODO: 这是你们的功课
			...
			let s_info = StorageValueRef::persistent(b"offchain-demo::polkadot-info");

			if let Ok(Some(polkadot_info)) = s_info.get::<PolkadotInfo>() {
				log::info!("#?#?#__cached polkadot-info: {:?}", polkadot_info);
				return Ok(());
			}

			let mut lock = StorageLock::<BlockAndTime<Self>>::with_block_and_time_deadline(
				b"offchain-demo::polkadot_lock", LOCK_BLOCK_EXPIRATION,
				rt_offchain::Duration::from_millis(LOCK_TIMEOUT_EXPIRATION)
				);

			if let Ok(_guard) = lock.try_lock() {
				match Self::fetch_n_parse_polkadot() {
					Ok(polkadot_info) => { s_info.set(&polkadot_info); }
					Err(err) => { return Err(err); }
				}
			}

			let polkadot_info = s_info.get::<PolkadotInfo>().unwrap().unwrap();
			log::info!("#?#?#__polkadot_info from s_info(after insert):{:?}", polkadot_info);
			let usd = polkadot_info.priceUsd;
			let price_usd_str = str::from_utf8(&usd).unwrap();
			let split: Vec<&str> = price_usd_str.split(".").collect();

			let split0 = split[0];
			let mut split1 = split[1];
			// 防止溢出
			if split1.len() > 7 {
				split1 = &split1[0..6];
			}
			log::info!("#?#?#__: split0:{:?} split1:{:?}", split0, split1);

			let split0u64: u64 = split0.parse().unwrap();
			let split1u32: u32 = split1.parse().unwrap();

			let price_integer: u64 = split0u64;
			let price_fraction = Permill::from_parts(split1u32);

			let result = (price_integer, price_fraction);
			log::info!("#?#?#__result********: {:?}", result);

			let signer = Signer::<T, T::AuthorityId>::any_account();
			if let Some((_, res)) = signer.send_unsigned_transaction(
				|account| PolkadotPayload { block_number, price_usd: result, public: account.public.clone() },
				Call::submit_polkadot_unsigned_with_signed_payload,
			) {
				return res.map_err(|_| {
					log::error!("Failed in offchain_unsigned_tx_signed_payload22");
		...
		fn fetch_n_parse_polkadot() -> Result<PolkadotInfo, Error<T>> {
			...
			log::info!("#?#?#__polkadot resp_str:{}", resp_str);

			// Deserializing JSON to struct, thanks to `serde` and `serde_derive`
			let resp_value: Value = serde_json::from_str(resp_str).unwrap();
			let data_object = resp_value.get("data").unwrap();
			let data_string = serde_json::to_string(data_object).unwrap();

			let polkadot_info: PolkadotInfo =
				serde_json::from_str(&data_string).map_err(|_| <Error<T>>::HttpFetchingError)?;
			log::info!("#?#?#__polkadot priceUsd:{:?}", polkadot_info);
			Ok(polkadot_info)
		}
		...
		fn fetch_from_remote_polkadot() -> Result<Vec<u8>, Error<T>> {
			log::info!("#?#?#__sending request to polkadot: {}", HTTP_REMOTE_REQUEST_POLKADOT);
			...
			if response.code != 200 {
				log::error!("#?#?#__Unexpected http request status code: {}", response.code);
